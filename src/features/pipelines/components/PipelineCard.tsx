import { useAppDispatch } from '@/app/hooks'

import { MAX_PIPELINE_RATE, MIN_PIPELINE_RATE } from '../constants'
import { pipelineLabel } from '../helpers'
import { pipelineRemoved, pipelineUpdated } from '../pipelinesSlice'
import type { Pipeline } from '../types'

interface PipelineCardProps {
  pipeline: Pipeline
  index: number
}

/** A created pipeline tier with inline-editable name and max throughput. */
export function PipelineCard({ pipeline, index }: PipelineCardProps) {
  const dispatch = useAppDispatch()
  const update = (changes: Parameters<typeof pipelineUpdated>[0]['changes']) =>
    dispatch(pipelineUpdated({ id: pipeline.id, changes }))

  return (
    <div className="flex items-center gap-3 rounded-lg border border-edge bg-surface-1 p-3">
      <input
        type="text"
        value={pipeline.name}
        placeholder={pipelineLabel(pipeline, index)}
        onChange={(e) => update({ name: e.target.value })}
        className="min-w-0 flex-1 rounded-md border border-edge bg-surface-0 px-2.5 py-1.5 text-sm text-gray-100 outline-none focus:border-ficsit"
      />
      <label className="flex shrink-0 items-center gap-1.5">
        <input
          type="number"
          min={MIN_PIPELINE_RATE}
          max={MAX_PIPELINE_RATE}
          step="any"
          value={pipeline.maxRate}
          onChange={(e) => update({ maxRate: Number(e.target.value) })}
          className="w-24 rounded-md border border-edge bg-surface-0 px-2 py-1 text-right font-mono text-sm text-gray-100 outline-none focus:border-ficsit"
          aria-label="Max rate (m³/min)"
        />
        <span className="text-xs text-gray-500">m³/min</span>
      </label>
      <button
        type="button"
        onClick={() => dispatch(pipelineRemoved(pipeline.id))}
        className="shrink-0 rounded px-2 py-1 text-xs text-red-400 transition hover:bg-red-500/15"
      >
        Delete
      </button>
    </div>
  )
}
